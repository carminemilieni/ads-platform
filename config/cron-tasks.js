...
const statsInserzioniBuild = async function () {
    const pid = await strapi.service('api::log.log').inizio('statsInserzioniBuild');

    try {

        const today = moment();
        const minToday = moment(today);
        const maxToday = moment(today);

        minToday.set('h', 0).set('m', 0).set('s', 0);
        maxToday.set('hours', 23).set('minutes', 59).set('seconds', 59)
        ...
        inserzioni.forEach(async (inserzione) => {
            const inserzioneDalDb = await strapi.db.query('api::inserzione.inserzione').findOne({ where: { id: inserzione.inserzione }, populate: ['posizioneAdv', 'campagna'] });
            await strapi.db.query('api::inserzione.inserzione').update({
                where: { id: inserzione.inserzione }, data: {
                    visualizzazioniUniche: inserzione.visualizzazioniUniche + parseInt(inserzioneDalDb.visualizzazioniUniche),
                    visualizzazioniTotali: inserzione.visualizzazioniTotali + parseInt(inserzioneDalDb.visualizzazioniTotali),
                    clicUnici: inserzione.clicUnici + parseInt(inserzioneDalDb.clicUnici),
                    clicTotali: inserzione.clicTotali + parseInt(inserzioneDalDb.clicTotali),
...
const statsCampagneBuild = async function () {
    ...
    try {
        ...
        const inserzioni = await strapi.db?.query('api::inserzione.inserzione').findMany({
            select: ['visualizzazioniUniche', 'visualizzazioniTotali', 'clicUnici', 'clicTotali'],
            populate: ['campagna'],
        });
        ...
        inserzioni.forEach(inserzione => {
            const index = campagne.findIndex(campagna => campagna.id === inserzione.campagna.id);
            if (index > -1) {
                campagne[index].visualizzazioniUniche += parseInt(inserzione.visualizzazioniUniche);
                campagne[index].visualizzazioniTotali += parseInt(inserzione.visualizzazioniTotali);
        ...
        campagne.forEach(async campagna => {
            ...
            const campagnaDalDb = await strapi.db.query('api::campagna.campagna').findOne({
                where: { id: campagna.id }, populate: ['cliente']
            });

            await strapi.db.query('api::campagna.campagna').update({
                where: { id: campagna.id }, data: {
...
const puliziaTracceObsolete = async function () {
    ...
    try {
        ...
        await strapi.service('api::log.log').fine(pid, `Eliminate ${result.count} tracce`);
